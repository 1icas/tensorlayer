...
__all__ = ['RTQuanConv2d']


class RTQuanConv2d(Layer):
    ...
    def __init__(
        ...
        super(RTQuanConv2d, self
             ).__init__(prev_layer=prev_layer, act=act, W_init_args=W_init_args, b_init_args=b_init_args, name=name)

        logging.info(
            "RTQuanConv2d %s: n_filter: %d filter_size: %s strides: %s pad: %s act: %s" % (
        ...
        with tf.variable_scope(name):
            ...
            scale = tf.get_variable(
                name='W_scale', shape=n_filter, initializer=W_init, dtype=LayersConfig.tf_dtype, **self.W_init_args
            )

            output_channel = self.outputs.get_shape()
            new_scale = tf.tile(scale, [output_channel[0]*output_channel[1]*output_channel[2]])
            new_scale = tf.reshape(new_scale, output_channel)
            self.outputs = tf.divide(self.outputs, new_scale)
        ...
        if b_init:
            self._add_params([W, b, scale])
        else:
            self._add_params([W, scale])
