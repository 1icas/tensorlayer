    ...
    def __init__(
        ...
        with tf.variable_scope(name):
            ...
            scale = tf.get_variable(
                name='W_scale', shape=n_units, initializer=W_init, dtype=LayersConfig.tf_dtype, **self.W_init_args
            )

            new_scale = tf.tile(scale, [self.outputs.get_shape()[0]])
            new_scale = tf.reshape(new_scale, [self.outputs.get_shape()[0], n_units])
